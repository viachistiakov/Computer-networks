...
func (client *Client) handleRequest(req *proto.Request) bool {
	...
	case "quit":
		...
		return true
	case "path":
		var nameOfDir string
		if err := json.Unmarshal(*req.Data, &nameOfDir); err != nil {
			client.logger.Println("addition failed", "reason", "Can't unmarshal")
			client.respond("failed", "Can't unmarshal")
		} else {
			if nameOfDir == "" {
				client.logger.Println("addition failed", "reason", "empty path")
				client.respond("failed", "empty path")
			} else {
				var res []string
				files, err := ioutil.ReadDir(nameOfDir)
				if err != nil {
					client.logger.Println("addition failed", "reason", err)
					client.respond("failed", err)
				}
...
func main() {
	...
	var addrStr string
	flag.StringVar(&addrStr, "addr", "127.0.0.1:5000", "specify ip address and port")
	flag.Parse()
